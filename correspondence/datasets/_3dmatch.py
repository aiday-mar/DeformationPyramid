...
from correspondence.lib.benchmark_utils import to_o3d_pcd, to_tsfm, KDTree_corr
from correspondence.lib.utils import load_obj
...
class _3DMatch(Dataset):
    ...
    def __getitem__(self, item, debug=False):
        ...
        if debug:
            ...
            scale_factor = 0.02
            # mlab.points3d(s_pc[ :, 0]  , s_pc[ :, 1],  s_pc[:,  2],  scale_factor=scale_factor , color=c_blue)
            mlab.points3d(src_pcd[ :, 0] , src_pcd[ :, 1], src_pcd[:,  2], resolution=8, scale_factor=scale_factor , color=c_red)
            mlab.points3d(tgt_pcd[ :, 0] , tgt_pcd[ :, 1], tgt_pcd[:,  2], resolution=8, scale_factor=scale_factor , color=c_blue)
            mlab.show()
