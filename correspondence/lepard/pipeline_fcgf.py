...
class Pipeline(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.backbone = FCGF(self.config.kpfcn_config)
    ...
    def forward(self, data, s_feats = None, t_feats = None, base = None, timers=None):
        
        self.timers = timers

        if self.timers: self.timers.tic('kpfcn backbone encode')
        coarse_feats = self.backbone(data, s_feats, t_feats, base, phase="coarse")
        ...
        src_feats, tgt_feats, src_pe, tgt_pe = self.coarse_transformer(src_feats, tgt_feats, s_pcd, t_pcd, src_mask, tgt_mask, data, timers=timers, mod = True)
