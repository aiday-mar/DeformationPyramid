...
def apply_simple_boundary_detection(file_path, num):
    ...
    dists = np.zeros((n_pcd_points, n_pcd_points))
    ...
    edge_points = np.array(pcd_points[initial_edge_point_indices])
    edge_points = np.squeeze(edge_points, axis=1)
    edge_points_pcd = o3d.geometry.PointCloud()
    edge_points_pcd.points = o3d.utility.Vector3dVector(edge_points)
    o3d.io.write_point_cloud('Testing/exterior_boundary_detection_initial_experimentation/simple_criterion_' + num + '.ply', edge_points_pcd)